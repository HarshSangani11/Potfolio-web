...
//           src={Logo}
//           alt="Logo"
//           className="h-64 w-56 md:h-14 lg:h-16 mx-auto hover:scale-105 transition-transform duration-300"
//         />
//       </span>
...
//           } mx-4 py-4 md:py-0 font-semibold bg-black bg-opacity-80 md:bg-transparent px-4 md:px-0 rounded-xl md:rounded-none text-center md:static md:flex gap-6 transition-all duration-500 transform ${menu ? "scale-100 opacity-100" : "scale-95 opacity-0"
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 flex flex-wrap justify-between items-center text-white px-6 py-4 bg-gradient-to-r from-gray-900 via-gray-800 to-black bg-opacity-90 shadow-lg">
      {/* Logo */}
      <span className="text-2xl font-bold tracking-wide hover:text-gray-300 transition duration-300">
        <img
          src={Logo}
          alt="Logo"
          className="h-12 w-auto md:h-14 lg:h-16 mx-auto hover:scale-105 transition-transform duration-300"
        />
      </span>
      ...
      {showMenu ? (
        <RiMenu2Line
          size={30}
          className="md:hidden absolute right-6 top-6 transition-transform duration-500 transform hover:scale-110 hover:rotate-90"
          onClick={() => {
            openMenu(!menu);
            setShowmenu(!showMenu);
          }}
